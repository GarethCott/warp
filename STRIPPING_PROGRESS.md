...
This doc is a snapshot of what's been done and what's left, designed so a fresh Claude session can pick up cold. Last updated 2026-05-01 (post-cleanup-72).
...
If you want a single concrete next step: all the unreachable settings pages are gone. After cleanups 55-72 the easy dead branches in `app/src/` are exhausted, the agent-mode setup banner subsystem is fully gone, the AskAI / AskAIAssistant chain across terminal/pane-group/workspace/ai_assistant has been deleted, the orphan `CommandSearchItemAction` AI variants are gone, the AI Assistant warm-welcome chain is fully removed, the `workspace:toggle_ai_assistant` bindings + their `NewPaneInAgentMode`/`ToggleAIAssistant` actions are gone, and the `workspace:add_current_dir_as_project` binding + `AddProjectAtCurrentDirectory` cascade is gone. Remaining iso targets are tougher:
...
- More dead `IS_ANY_AI_ENABLED`-gated EditableBindings remain in `terminal/view/init.rs` (~10), `terminal/input.rs` (3), and `workspace/mod.rs` (5+). Cleanups 71 and 72 demonstrated the pattern. Each one needs careful checking that the underlying action has no other producers — many DO have live producers (URI handlers, slash commands, programmatic dispatchers in the dead-on-arrival ai/ subtree that still compiles). For example: `workspace:init_project_rules` → `TerminalAction::InitProject` is NOT clean (live producers: `slash_commands/mod.rs` `/init`, `code_review_view.rs` `InitProjectForCurrentDirectory` button), and `EnterCloudAgentView` is NOT clean (live producer: `slash_commands/mod.rs:410` event emit). Look for variants whose only construction sites are gated bindings. Other candidates to vet: `terminal:toggle_autoexecute_mode_keybinding`, `terminal:toggle_queue_next_prompt_keybinding`.
