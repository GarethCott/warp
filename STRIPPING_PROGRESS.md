...
This doc is a snapshot of what's been done and what's left, designed so a fresh Claude session can pick up cold. Last updated 2026-05-01 (post-cleanup-74).
...
If you want a single concrete next step: all the unreachable settings pages are gone. After cleanups 55-74 the easy dead branches in `app/src/` are exhausted, the agent-mode setup banner subsystem is fully gone, the AskAI / AskAIAssistant chain across terminal/pane-group/workspace/ai_assistant has been deleted, the orphan `CommandSearchItemAction` AI variants are gone, the AI Assistant warm-welcome chain is fully removed, the `workspace:toggle_ai_assistant` + `workspace:add_current_dir_as_project` + `workspace:create_personal_ai_prompt` + `workspace:create_team_ai_prompt` + `workspace:jump_to_latest_toast` + `workspace:toggle_agent_management_view` bindings and their cascades are gone, and the `AgentManagement` header-toolbar item + its full state plumbing is gone. Remaining iso targets are tougher:
