...
This doc is a snapshot of what's been done and what's left, designed so a fresh Claude session can pick up cold. Last updated 2026-05-01 (post-cleanup-75).
...
If you want a single concrete next step: all the unreachable settings pages are gone. After cleanups 55-75 the easy dead branches in `app/src/` are exhausted, the agent-mode setup banner subsystem is fully gone, the AskAI / AskAIAssistant chain across terminal/pane-group/workspace/ai_assistant has been deleted, the orphan `CommandSearchItemAction` AI variants are gone, the AI Assistant warm-welcome chain is fully removed, all the `IS_ANY_AI_ENABLED`-gated workspace + input bindings (`toggle_ai_assistant`, `add_current_dir_as_project`, `create_personal_ai_prompt`, `create_team_ai_prompt`, `jump_to_latest_toast`, `toggle_agent_management_view`, `toggle_natural_language_command_search`) and their cascades are gone, the `AgentManagement` header-toolbar item + its full state plumbing is gone, and the AI command-search telemetry chain is dropped. Remaining iso targets are tougher:
