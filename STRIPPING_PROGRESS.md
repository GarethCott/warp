...
This doc is a snapshot of what's been done and what's left, designed so a fresh Claude session can pick up cold. Last updated 2026-05-01 (post-cleanup-61).
...
**Total stripped: ~37100+ lines of dead code, 49 files entirely deleted, 18 dead feature flags removed.**
...
If you want a single concrete next step: all the unreachable settings pages are gone, and after cleanups 55-61 the easy `if false`/`&& false` dead branches in `app/src/` are basically exhausted (only the comment-marker in `workspace/view.rs:2828` remains in the grep). Remaining iso targets are tougher:
...
- The `agent_mode_setup_speedbump_banner` action enum + dispatch wiring (cleanup-61 left it intact). The action can never fire — `AgentModeSetupSpeedbumpBannerAction` and the `agent_setup_speedbump_banner` field could come out together with `terminal/view/action.rs:366`, the field at `view.rs:1104`, and the `InlineBannerType::AgentModeSetup` arm.
