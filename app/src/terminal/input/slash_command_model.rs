...
impl SlashCommandModel {
    pub fn new(
        ...
        // strip(neuter): AgentView is gated off in this fork; slash commands
        // are disabled in locked shell mode.
    ...
    fn handle_input_buffer_update(
        ...
        match self.detect_command(new, ctx) {
            SlashCommandEntryState::SlashCommand(detected_command) => {
                ...
                // force AI input type when a slash command is detected.
            ...
            _ if new.starts_with('/') => {
                ...
                // strip(neuter): AgentView is gated off in this fork; always
                // force AI input type when composing a slash command.
                self.ai_input_model.update(ctx, |input_model, ctx| {
                    input_model.set_input_type(InputType::AI, ctx);
                });
