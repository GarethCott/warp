...
impl ConversationOrTask<'_> {
    ...
    fn link_preference(&self) -> LinkPreference {
        match self {
            ConversationOrTask::Task(task) => {
                ...
                // strip(neuter): CloudConversations is gated off in this fork; always open
                // the session link when one is available and not expired.
    ...
    pub fn get_session_status(&self) -> Option<SessionStatus> {
        // strip(neuter): CloudConversations is gated off in this fork.
        match self {
            ConversationOrTask::Task(task) => {
