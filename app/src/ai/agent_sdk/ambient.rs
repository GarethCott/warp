...
use warp_cli::{
    agent::{OutputFormat, RunCloudArgs},
...
impl AmbientAgentRunner {
    ...
    fn run_agent(&self, _args: RunCloudArgs, _ctx: &mut ModelContext<Self>) -> anyhow::Result<()> {
        // strip(neuter): AmbientAgentsCommandLine is gated off in this fork.
        Err(anyhow::anyhow!("Unsupported feature"))
    }
