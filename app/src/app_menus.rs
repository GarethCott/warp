...
fn make_new_elements_menu_items(ctx: &AppContext) -> Vec<MenuItem> {
    ...
    let mut new_elements_menu = vec![
        ...
        // strip(neuter): "New Terminal Tab" no longer needs to switch
        // its trigger based on default session mode (agent mode is gone).
        MenuItem::Custom(CustomMenuItem::new(
            "New Terminal Tab",
        ...
        // strip(neuter): "New Agent Tab" removed - agent mode is gone.
